...
function lose(userChoice, computerChoice) {
  ...
  userScore_span.innerHTML = userScore;
  computerScore_span.innerHTML = computerScore;
  result_p.innerHTML = `${convetToWord(userChoice)}${smallUserWord} loses to ${convetToWord(computerChoice)}${smallCompWord}. You Lost...`;
...
function draw(userChoice, computerChoice) {
  ...
  const smallCompWord = "comp".fontsize(3).sub();
  const userChoice_div = document.getElementById(userChoice);
  result_p.innerHTML = `${convetToWord(userChoice)}${smallUserWord} equals ${convetToWord(computerChoice)}${smallCompWord}. It's a draw.`;
